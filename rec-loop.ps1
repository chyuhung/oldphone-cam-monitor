...
if (-not (Test-Path $ffmpeg)) {
    Write-Host 'ffmpeg.exe not found at vendor\ffmpeg\ffmpeg.exe'
    Write-Host 'Run download.bat once (or get ffmpeg.exe from the GitHub Releases page).'
    exit 1
}
...
if (-not $url) {
    Write-Host 'streams.phone1 not found in config.yaml - edit config.yaml first.'
    exit 1
}
...
Add-Content -Path $log -Value ($ts + '  recorder start, url=' + $url)
